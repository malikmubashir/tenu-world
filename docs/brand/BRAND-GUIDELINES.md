...
Change log.
v2.1 addendum (2026-06-10 evening) · **Navy/gold accent layer + conversion hero rule (owner override of the achromatic rule, #T167).** Inversion bands and the filled-CTA exception shift from absolute black to deep navy `#0B1F3A`; links carry navy ink `#1E4477` (always underlined, so they stay distinguishable from black body text). Gold `#C4A35A` (`#D4B86A` on navy bands) enters for **lines and micro-details only** — the 1px rule above section-opening kickers (`.ed-kicker`), the single signature rule at the top of the footer (`.ed-rule-gold`), and the key-value circle line-art strokes. Gold is never a fill and never text below 14px bold (contrast fails on white); restraint target is 4–6 gold occurrences per page. **Conversion hero rule:** the homepage hero is a split layout whose first viewport contains everything — kicker, display headline clamped to ~64–76px (`.ed-display-hero`), one-line subtext, filled CTA + ghost link, and the photographic plate. Desktop: text ~45% inline-start, full-height plate ~55% inline-end, total ≈ 100svh minus header. Mobile: plate ~38svh on top, text + CTA below within the first scroll. Tokens live in `src/app/theme.css`; classes in `src/app/globals.css`. Canvas stays white, ink stays black, wordmark unchanged.
