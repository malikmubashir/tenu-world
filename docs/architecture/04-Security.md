...
**Policy wording aligned 2026-06-11 (#T185, docs/17 finding 3).** The privacy policy (FR + EN §5, amended 2026-06-11, pending DPO review with V3 #T133) no longer claims that no transfer outside the EU occurs. It now discloses the actual posture: data hosted in the EU (Supabase Frankfurt, Cloudflare R2 EU, Vercel CDG), with photographs and tenancy details processed during AI analysis by Anthropic (USA) as sub-processor under Zero Data Retention (no storage after processing, no training use) and EU standard contractual clauses, moving to AWS Bedrock (Frankfurt) post-launch.

Branch `feat/bedrock-migration` moves both AI modules to AWS Bedrock Frankfurt (`eu-central-1`) with an `AWS_REGION` must-start-with-`eu-` boot guard, after which `ANTHROPIC_API_KEY` is removed from Vercel. It does NOT merge before the 15 Jul launch. Until it merges and MH completes the AWS account/IAM/model-access checklist, **`main` still sends inspection photos and tenancy details to `api.anthropic.com`**. The compensating control the policy wording now relies on is tracked as #T132 (owner: MH): enable Zero Data Retention on the Anthropic org with dated screenshot evidence — this must be in force before paid signups open beyond F&F, otherwise the amended policy overstates the safeguard.
...
- **Art. 17 erasure**: `POST /api/account/delete` with typed-email confirmation; as of 2026-06-11 the route purges the user's R2 objects **before** the DB cascade (`src/lib/storage/r2-erasure.ts`: DB-recorded keys ∪ full `userId/` prefix listing, batched `DeleteObjects`, idempotent on missing objects, partial failures logged for manual sweep without blocking erasure), then service-role `deleteUser` + FK cascade. See §8.5 (resolved).
...
5. **Erasure does not purge R2 — resolved 2026-06-11 (#T185, docs/17 finding 3).** `/api/account/delete` now purges R2 before the DB cascade via `src/lib/storage/r2-erasure.ts`: enumerates `photos.r2_key`, `inspections.contract_pdf_r2_key`, the scan-report PDF (`risk_score.pdfUrl`) and `dispute_letters.letter_pdf_url`, unions that with a full `ListObjectsV2` sweep of the `userId/` key prefix (all upload paths are user-namespaced), deletes in ≤1000-key `DeleteObjects` batches. Missing objects count as success (idempotent); partial failures are collected and logged (`[account-delete] R2 purge incomplete`) for a manual sweep bounded by the `userId/` prefix, and never block the DB erasure. Unit-tested in `src/lib/storage/r2-erasure.test.ts`. Residual: the 24-month *retention-period* purge for PDFs (§7 table) remains policy-only — this fix covers user-requested erasure, not scheduled expiry.
