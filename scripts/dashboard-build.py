...
def parse():
    """Parse TASKS.md. Each task line carries a permanent #TNNN ID injected by retrofit-ids.py."""
    ...
    for line in raw.splitlines():
        ...
        id_m = re.match(r'#T(\d{3})\s+(.+)$', body)
        if id_m:
            task_id = f"T{id_m.group(1)}"
            body = id_m.group(2)
        else:
            task_id = '???'  # ID missing — alert in dashboard
        ...
        items.append({'section':section,'owner':owner,'title':title,'priority':prio,
                      'due':due,'status':status,'note':note,'task_id':task_id})
    return items
...
def render(items):
    ...
    def render_item(it):
        ...
        tid = it['task_id']
        return f'''
        <article class="item {cls}" id="task-{tid}">
          <header>
...
{''.join(f'<a href="#task-{it["task_id"]}" title="{esc(it["title"][:80])}">#{it["task_id"]}</a>' for it in sorted(items, key=lambda x: x['task_id']))}
</div>
...
<footer>
  ...
  <div>{len(items)} tasks · {overdue} overdue · {in_prog} in progress · permanent IDs (next available in <code>.tasks-id-counter</code>)</div>
</footer>
