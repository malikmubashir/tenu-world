 ...
 *
 * With static export on Next 15 dynamic [id] routes work only if
 * generateStaticParams returns a non-empty array. For mobile we don't
 * know ids at build time, so this page is rendered via client-side
 * routing only — the URL is built after draft creation. The build
 * script passes `dynamicParams: true` implicitly; if a static-export
 * build complains, set `export const dynamic = 'force-dynamic'` at
 * build time and let the client fill in.
 */
